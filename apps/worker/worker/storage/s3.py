"""S3 storage backend for worker. Uploads to AWS S3; returns public or custom-base URL."""
...
class S3Storage:
    ...
    def __init__(
        ...
        *,
        ...
        self.bucket = (bucket or settings.s3_bucket).strip()
        self.region = (region or settings.s3_region).strip()
    ...
    def put(self, key: StorageKey, data: bytes, content_type: str) -> str:
        ...
        try:
            ...
            logger.exception("Worker S3 put_object failed")
            raise RuntimeError(f"S3 upload failed: {e!s}") from e
        return self.url_for(key)

    def url_for(self, key: StorageKey) -> str:
        """Return public URL for the object (for preview in extension/web)."""
        if self.base_url:
